...
def home(l):
    s=f'''<section class="hero-v2" aria-labelledby="identity-title"><div class="hero-atmosphere" aria-hidden="true"></div><div class="hero-shade" aria-hidden="true"></div><div class="hero-content"><p class="eyebrow"><span class="status-dot"></span>{tx("CUHK · Electronic Engineering · PhD Researcher","香港中文大学 · 电子工程 · 博士研究生",l)}</p><h1 id="identity-title">{tx("Shurong<br><em>Cao.</em>","曹书嵘<span class=hero-english>Shurong Cao.</span>",l)}</h1><p class="hero-thesis">{tx("Small devices.<br><span>New dimensions.</span>","从微小器件，<br><span>探索新的维度。</span>",l)}</p><p class="hero-description">{tx("Exploring semiconductor devices, low-temperature fabrication and monolithic 3D integration.","探索半导体器件、低温制造与单片三维集成，连接材料、工艺与电子系统。",l)}</p><div class="hero-actions">{link("#research-interests",tx("Explore my research","探索我的研究",l)+" <span>↗</span>","button light")}{link("https://github.com/CAOShurong","GitHub Profile <span>↗</span>","button glass")}</div></div><div class="hero-side"><span>01 / {tx("THE NEXT DIMENSION","探索新的维度",l)}</span><div class="hero-signature">{tx("Devices. Materials. Possibilities.","器件 · 材料 · 可能性",l)}</div></div><a class="scroll-cue" href="#about"><span>↓</span>{tx("Meet the researcher","了解我的研究旅程",l)}</a><button class="motion-toggle" type="button" aria-pressed="false">{tx("Pause motion","暂停动效",l)} <span aria-hidden="true">Ⅱ</span></button></section>'''
    s+=f'''<section id="about" class="about-v2 section"><div class="about-portrait" data-reveal>{portrait(l)}<span>{tx("SHURONG CAO / 曹书嵘","曹书嵘 / SHURONG CAO",l)}</span></div><div class="about-copy" data-reveal><p class="eyebrow">{tx("A researcher. An engineer. A builder.","研究者 · 工程师 · 开源贡献者",l)}</p><h2>{tx("Curiosity at the device level.<br><em>Perspective at the system level.</em>","从器件出发，<br><em>用系统的视角探索。</em>",l)}</h2><p>{tx("I am a PhD student in Electronic Engineering at The Chinese University of Hong Kong, advised by Prof. Ni Zhao. My research is taking shape around the materials, processes and architectures that could bring new functionality above existing silicon.","我是香港中文大学电子工程博士研究生，导师为 Prof. Ni Zhao。目前围绕材料、工艺与器件结构展开探索，关注如何在现有硅基电路之上实现新的功能。",l)}</p><p class="muted">{tx("An integrated-circuit foundation from Nanjing University. Experience across physical simulation, industrial vision, intelligent hardware and open-source research software.","本科就读于南京大学集成电路学院。此前的研究与实践横跨物理仿真、工业视觉、智能硬件与开源科研软件。",l)}</p><div class="text-links">{link(url("cv",l),tx("Curriculum vitae","个人简历",l)+" ↗")}{link("mailto:"+EMAIL,tx("Get in touch","邮件联系",l)+" ↗")}</div></div></section>'''
    s+=f'<section class="research-v2 section" id="research-interests"><div class="section-heading" data-reveal><div><p class="eyebrow">{tx("Research horizons","研究视野",l)}</p><h2>{tx("Beyond the<br><em>silicon plane.</em>","走向硅平面<br><em>之上的可能性。</em>",l)}</h2></div><p>{tx("Three connected questions are shaping my current exploration: how we build, which materials we use, and how devices connect in a new dimension.","如何制造、选择怎样的材料、如何在新的维度连接器件——三个相互关联的问题，构成我当前的探索。",l)}</p></div><div class="research-explorer" data-research-explorer><div class="research-selectors" aria-label="{tx("Research directions","研究方向",l)}">'
    for i,r in enumerate(RESEARCH):
        s+=f'<button type="button" data-research="{i}" aria-controls="research-panel-{i}" aria-expanded="true"><span class="selector-index">0{i+1}</span><span>{r[2 if l else 1]}<small>{r[4 if l else 3]}</small></span><span class="selector-arrow">↗</span></button>'
    s+='</div><div class="research-stage">'
    for i,(r,v) in enumerate(zip(RESEARCH,RESEARCH_VISUALS)):
        s+=f'<article id="research-panel-{i}" class="research-panel"><div class="research-stage-figure">{research_figure(v,l)}</div><div class="research-stage-copy"><p class="eyebrow">{v["tags"][l]}</p><h3>{r[2 if l else 1]}</h3><p>{r[6 if l else 5]}</p>{link(url("research",l)+"#theme-"+r[0],tx("Explore the questions","深入了解这一方向",l)+" ↗","arrow-link")}</div></article>'
    s+=f'</div></div><p class="literature-note">{tx("Visual references from selected literature; sources accompany each figure.","代表性文献图，出处见各图下方。",l)}</p></section>'
    s+=f'<section class="publications-v2 section" id="selected-publications"><div class="section-heading" data-reveal><div><p class="eyebrow">{tx("Selected publications","代表论文",l)}</p><h2>{tx("From questions<br><em>to contributions.</em>","让问题，<br><em>成为研究成果。</em>",l)}</h2></div>{link(url("publications",l),tx("Publication record","查看论文列表",l)+" ↗","arrow-link")}</div><div class="publication-features">'+''.join(paper(p,l,True) for p in PAPERS)+'</div></section>'
    s+=f'<section class="practice-v2 section" id="research-practice"><div class="section-heading" data-reveal><div><p class="eyebrow">{tx("Research into practice","从研究到实践",l)}</p><h2>{tx("Ideas are only<br><em>the beginning.</em>","让想法，<br><em>走进真实世界。</em>",l)}</h2></div><p>{tx("Simulation to hardware. Device concepts to research tools. Selected projects where understanding becomes something tangible.","从仿真到硬件，从器件概念到科研工具。在具体项目中，把理解转化为可以运行、可以验证的成果。",l)}</p></div><div class="project-mosaic">'+''.join(project_card(p,l) for p in PROJECTS)+'</div></section>'
    s+=f'<section class="opensource-v2" id="open-source"><div class="open-source-intro" data-reveal><p class="eyebrow">{tx("Open-source engineering","开源工程",l)}</p><h2>{tx("Built to work.<br><em>Shared to go further.</em>","把工具做好，<br><em>与社区走得更远。</em>",l)}</h2><p>{tx("I maintain BenchLineage and contribute to the tools that researchers and developers rely on.","我维护 BenchLineage，也参与改进研究者与开发者日常依赖的开源工具。",l)}</p>{link("https://github.com/CAOShurong","GitHub Profile ↗","button light")}</div><div class="open-source-evidence"><div class="contribution-counts"><div><strong>37</strong><span>{tx("Merged upstream PRs","已合并的上游 PR",l)}</span></div><div><strong>22</strong><span>{tx("External repositories","外部开源仓库",l)}</span></div></div><div class="contribution-list">'+''.join(f'<a href="{u}"><span>{n}</span><span>↗</span></a>' for n,u,_,_ in PRS[:3])+f'</div><div class="evidence-footer"><span>{tx("September 2026 snapshot","2026 年 9 月快照",l)}</span>{link(url("projects",l)+"#upstream",tx("Explore contributions","查看贡献",l)+" ↗")}</div></div></section>'
    s+=f'<section class="journey-v2 section" id="academic-exchange"><div class="section-heading" data-reveal><div><p class="eyebrow">{tx("An expanding perspective","不断拓展的视野",l)}</p><h2>{tx("Rooted in engineering.<br><em>Open to the world.</em>","立足工程，<br><em>向世界学习。</em>",l)}</h2></div>{link(url("experience",l),tx("Experience & recognition","经历与荣誉",l)+" ↗","arrow-link")}</div>{education(l)}{exchanges(l)}</section>'
    s+=f'<section class="contact-v2"><p class="eyebrow">{tx("Research · Engineering · Collaboration","研究 · 工程 · 合作",l)}</p><h2>{tx("What could we<br><em>explore together?</em>","下一种可能，<br><em>一起探索。</em>",l)}</h2>{link("mailto:"+EMAIL,EMAIL+" <span>↗</span>","contact-invitation")}</section>'
    return s
...
def shell(path,l,body,title):
    ...
    for p,en,zh in NAV:
        ...
        nav+=item
    hero_preload='<link rel="preload" as="image" href="/assets/hero-frontier.webp">' if not path else ''
