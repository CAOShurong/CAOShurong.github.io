...
<nav class="page-index" aria-label="{tx("On this page","本页目录",l)}"><span>{tx("On this page","本页内容",l)}</span>{link("#research-interests",tx("Research interests","研究兴趣",l))}{link("#selected-publications",tx("Publications","论文",l))}{link("#research-practice",tx("Projects","项目",l))}{link("#open-source",tx("Open source","开源",l))}{link("#academic-exchange",tx("Background","学术经历",l))}</nav>'''
    ...
    tail+=f'<section class="opensource-home section" id="open-source">{section(tx("Open-source work","开源工作",l),"GitHub Profile","https://github.com/CAOShurong")}<p>{tx("I maintain <a href=https://github.com/CAOShurong/benchlineage>BenchLineage</a>, a provenance toolkit for engineering experiments, and contribute to scientific computing and developer tools.","我维护面向工程实验的溯源工具 <a href=https://github.com/CAOShurong/benchlineage>BenchLineage</a>，也参与科学计算与开发者工具的上游协作。",l)}</p><div class="contribution-summary"><p><strong>37</strong> {tx("merged upstream PRs","个上游已合并 PR",l)}</p><p><strong>22</strong> {tx("external repositories","个外部开源仓库",l)}</p></div><div class="pr-list">'+''.join(f'<article>{link(p[1],p[0]+" ↗")}<p>{p[3 if l else 2]}</p></article>' for p in PRS)+f'</div><div class="evidence-footer"><span>{tx("September 2026 snapshot","2026 年 9 月快照",l)}</span>{link(url("projects",l)+"#upstream",tx("Contribution record","查看贡献记录",l)+" ↗")}</div></section>'
    s+=f'<aside class="academic-rail" id="academic-exchange"><section><h2>{tx("Education","教育背景",l)}</h2>{education(l)}</section><section><h2>{tx("Academic exchanges","学术交流",l)}</h2>{exchanges(l)}</section><section class="rail-recognition"><h2>{tx("Selected recognition","代表荣誉",l)}</h2><p><strong>{tx("National Second Prize","全国二等奖",l)}</strong><span>{tx("FPGA Innovation Design · 2024","FPGA 创新设计赛道 · 2024",l)}</span></p><p><strong>{tx("People’s Scholarship","人民奖学金",l)}</strong><span>{tx("Nanjing University · 2023, 2024","南京大学 · 2023、2024",l)}</span></p>{link(url("experience",l),tx("Experience & recognition","经历与荣誉",l)+" ↗")}</section><section class="rail-contact"><h2>{tx("Connect","联系",l)}</h2><p>{tx("Research conversations, engineering collaborations and open-source ideas are welcome.","欢迎交流研究问题、工程合作与开源想法。",l)}</p>{link("mailto:"+EMAIL,EMAIL+" ↗")}<p class="masked">{MASKED}</p></section></aside></div>'
    return s+tail
