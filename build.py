...
CUHK_EE='https://www.ee.cuhk.edu.hk/'
NI_GROUP='https://www.ee.cuhk.edu.hk/~nzhao/'
...
def paper(p,l,full=False):
    ...
    return f'<article class="paper" data-reveal id="{p["id"]}"><a class="paper-figure" href="{figure_url}" aria-label="{esc(p["title"])}">{img(display_image,display_alt)}</a><div><div class="paper-top"><span class="eyebrow">{link(p["links"][0][1],p["short"],"quiet-link")}</span>{short}</div><h3>{link(p["links"][0][1],p["title"],"paper-title-link")}</h3><p class="authors">{p["authors"]}</p><p class="venue">{p["venue"]}</p>{body}<div class="text-links">{links}</div></div></article>'
...
def research_grid(l):
    ...
    for r,v in zip(RESEARCH,RESEARCH_VISUALS):
        cards.append(f'<article class="research-card"><h3>{link(url("research",l)+"#theme-"+r[0],r[2 if l else 1])}</h3>{research_figure(v,l)}<div class="research-card-copy"><p class="eyebrow">{v["tags"][l]}</p><p>{r[6 if l else 5]}</p>{link(url("research",l)+"#theme-"+r[0],tx("Explore this direction","了解这一方向",l)+" ↗","direction-link")}</div></article>')
    return '<div class="research-grid">'+''.join(cards)+'</div>'

def exchanges(l):
    return f'<div class="exchange-grid"><article><a class="exchange-logo" href="https://www.hkust-gz.edu.cn/">{img("hkust-gz-logo.png",tx("HKUST Guangzhou logo","香港科技大学（广州）校徽与校名",l))}</a><div><p class="eyebrow">{tx("Research experience","科研交流",l)}</p><h3>{link("https://www.hkust-gz.edu.cn/",tx("HKUST (Guangzhou)","香港科技大学（广州）",l),"quiet-link")}</h3><p>{tx("Visiting research in nanophotonics and optoelectronic devices, connecting simulation, geometry optimization and device characterization.","围绕纳米光子学与光电器件开展科研交流，连接仿真、结构优化与器件表征。",l)}</p></div></article><article><a class="exchange-logo" href="https://www.cam.ac.uk/">{img("cambridge-logo.svg",tx("University of Cambridge logo","剑桥大学校徽与校名",l))}</a><div><p class="eyebrow">{tx("Academic exchange","学术交流",l)}</p><h3>{link("https://www.cam.ac.uk/",tx("Cambridge · St Catharine’s College","剑桥 · 圣凯瑟琳学院",l),"quiet-link")}</h3><p>{tx("Hardware design programme; team leader and commencement speaker. A collaborative engineering experience in an international academic setting.","参加硬件设计交流项目，担任团队负责人及结业发言人，在国际学术环境中开展协作式工程实践。",l)}</p></div></article></div>'

def education(l):
    return f'<div class="education"><article><span class="school-crest cuhk-crest"><img src="/assets/cuhk-logo.png" alt="CUHK crest"></span><div><span class="date">2026 — {tx("present","至今",l)}</span><h3>{link(CUHK,tx("The Chinese University of Hong Kong","香港中文大学",l),"institution-link")}</h3><p>{tx("PhD student · Electronic Engineering","电子工程博士研究生",l)}</p><p class="muted">{tx("Supervisor: ","导师：",l)}{link(NI_GROUP,"Prof. Ni Zhao","supervisor-link")}</p></div></article><article><span class="school-crest"><img src="/assets/nju-crest.svg" alt="Nanjing University crest"></span><div><span class="date">2022 — 2026</span><h3>{link(NJU,tx("Nanjing University","南京大学",l),"institution-link")}</h3><p>{tx("B.Eng. · Integrated Circuit and System Design","工学学士 · 集成电路设计与集成系统",l)}</p><p class="muted">{tx("School of Integrated Circuits · Suzhou Campus","集成电路学院 · 苏州校区",l)}</p></div></article></div>'
...
def project_card(p,l):
    ...
    return f'<article class="project-card" data-reveal><a class="project-visual" href="{url("projects/"+p["id"],l)}" aria-label="{esc(p["title"][l])}">{visual}</a><div class="project-content"><p class="eyebrow">{p["tag"][l]}</p><h3>{link(url("projects/"+p["id"],l),p["title"][l]+"&nbsp;↗")}</h3><p>{p["summary"][l]}</p></div></article>'


def institutional_note(l):
    return tx("To continue a conversation through my university email, please reach out via Gmail first.","如需通过我的学校邮箱联系，请先发邮件至 Gmail，我们再进一步沟通。",l)

def recognition(l):
    return [(tx("Guo Xie Birong Scholarship","郭谢碧蓉奖学金",l),tx("Nanjing University · RMB 10,000","南京大学 · ¥10,000",l)),(tx("National Second Prize","全国二等奖",l),tx("FPGA Innovation Design · 2024","FPGA 创新设计赛道 · 2024",l))]

def official_credits(l, full=False):
    title=tx('Contributions','开源贡献',l)
    intro=tx('Research software I maintain, and improvements to the open-source tools we rely on.','维护科研软件，也改进研究与工程所依赖的开源工具。',l)
    top=heading(tx('Open source','开源实践',l),title,intro) if full else section(title,tx('All contributions','全部贡献',l),url('recognition',l))+f'<p class="credit-intro">{intro}</p>'
    profile=f'<a class="contributor-profile" href="https://github.com/CAOShurong"><span><span class="eyebrow">GitHub · @CAOShurong</span><strong>{tx("Explore my GitHub","访问我的 GitHub",l)}</strong></span><span>{tx("Repositories, contributions & ongoing work","仓库、贡献与持续进行的工作",l)} <b aria-hidden="true">↗</b></span></a>'
    extra=''
    if full:
        extra=section(tx('Software & scientific computing','科研软件与科学计算',l))
        extra+='<div class="contribution-showcase">'
        for c in OPEN_SOURCE_FEATURES:
            if c['name']=='GitHub MCP Server': continue
            extra+=f'<a class="contribution-card" href="{c["url"]}"><div class="contribution-shot">{img(c["image"],c["alt"][l])}</div><div class="contribution-copy"><p class="eyebrow">{c["role"][l]}</p><h3>{c["name"]} ↗</h3><p>{c["summary"][l]}</p></div></a>'
        extra+='</div>'
    return top+profile+credit_cards(l, full)+extra


def home(l):
    s=f'''<section class="identity" id="about" aria-labelledby="identity-title">
<div class="identity-heading"><p class="eyebrow">{link(CUHK_EE,tx("Electronic Engineering · CUHK","电子工程 · 香港中文大学",l),"quiet-link")}</p><a class="identity-name" href="{url("experience",l)}"><h1 id="identity-title">{tx("Shurong Cao","曹书嵘",l)}<span>{tx("曹书嵘","Shurong Cao",l)}</span></h1></a><p class="identity-role">{link(url("research",l),tx("PhD student · Semiconductor devices & integration","博士研究生 · 半导体器件与集成",l),"quiet-link")}</p></div>
<div class="identity-portrait">{portrait(l)}<span>{tx("Hong Kong","香港",l)} · 2026</span></div>
<div class="identity-bio"><p>{tx("I am a PhD student in Electronic Engineering at <strong><a class='prose-link' href='https://www.cuhk.edu.hk/'>The Chinese University of Hong Kong</a></strong>, advised by <strong><a class='prose-link' href='https://www.ee.cuhk.edu.hk/~nzhao/'>Prof. Ni Zhao</a></strong>.","我是<strong><a class='prose-link' href='https://www.cuhk.edu.hk/'>香港中文大学</a></strong>电子工程博士研究生，导师为 <strong><a class='prose-link' href='https://www.ee.cuhk.edu.hk/~nzhao/'>Prof. Ni Zhao</a></strong>。目前探索半导体器件与低温工艺，关注如何在现有硅基电路之上实现新的功能。",l)}</p><p>{tx("I explore <strong><a class='prose-link' href='/research/#theme-01'>BEOL-compatible devices</a></strong>, <strong><a class='prose-link' href='/research/#theme-02'>p-type oxides</a></strong> and <strong><a class='prose-link' href='/research/#theme-03'>monolithic 3D integration</a></strong>, with an interest in low-temperature fabrication. My background at Nanjing University spans integrated circuits, device simulation, industrial vision and intelligent hardware.","当前兴趣连接 <strong><a class='prose-link' href='/zh/research/#theme-01'>BEOL 兼容器件</a></strong>、<strong><a class='prose-link' href='/zh/research/#theme-02'>p 型氧化物</a></strong>与<strong><a class='prose-link' href='/zh/research/#theme-03'>单片三维集成</a></strong>。本科就读于南京大学集成电路学院，此前的研究与实践涵盖器件仿真、工业视觉与智能硬件。",l)}</p><div class="identity-links">{link("https://github.com/CAOShurong","GitHub Profile <span>↗</span>","button")}{link("mailto:"+EMAIL,tx("Email me","邮件联系",l)+" ↗")}</div><a class="identity-email" href="mailto:{EMAIL}">{EMAIL}</a></div></section>
<nav class="page-index" aria-label="{tx("On this page","本页目录",l)}"><span>{tx("On this page","本页内容",l)}</span>{link("#research-interests",tx("Research interests","研究兴趣",l))}{link("#selected-publications",tx("Publications","论文",l))}{link("#research-practice",tx("Projects","项目",l))}{link("#recognition",tx("Contributions","开源贡献",l))}{link("#academic-exchange",tx("Background","学术经历",l))}</nav>'''
    ...
    tail+=f'<section class="section official-credits" id="recognition"><span id="open-source"></span>{official_credits(l)}</section>'
...
def research(l):
    s=heading(tx('Research','研究',l),tx('Research interests','研究兴趣',l),tx('My research direction is developing around semiconductor devices and practical routes to vertically integrated electronics.','我的研究方向正在围绕半导体器件与垂直集成电子系统的实际工艺路径逐步展开。',l))
...
def publications(l):return heading(tx('Publications','论文',l),tx('Publications','论文',l),tx('Research in wafer inspection and industrial anomaly detection.','晶圆检测与工业异常检测中的研究工作。',l))+''.join(paper(p,l,True) for p in PAPERS)

def projects(l):
    s=heading(tx('Projects','项目',l),tx('Research & engineering','研究与工程实践',l),tx('Selected work across devices, simulation, intelligent hardware, and software for research.','器件、仿真、智能硬件与科研软件中的代表工作。',l))
    ...
    s+=f'<p id="upstream">{link(url("recognition",l),tx("Explore upstream contributions","浏览上游开源贡献",l)+" ↗")}</p>'
...
def experience(l):
    s=heading(tx('Experience','经历',l),tx('Education & experience','教育与经历',l))
    ...
    s+=f'<section class="section">{section(tx("Selected recognition","代表性荣誉",l))}<ul class="recognition">{''.join(f'<li><strong>{name}</strong><p>{detail}</p></li>' for name,detail in recognition(l))}</ul></section>'
...
def cv(l):
    ...
    s+=f'<section class="section">{section(tx("Publications","论文",l))}'+''.join(f'<article class="cv-publication"><h3>{link(p["links"][0][1],p["title"],"paper-title-link")}</h3><p>{p["authors"]}</p><p>{p["venue"]}</p></article>' for p in PAPERS)+'</section>'
    ...
    s+=f'<section class="section">{section(tx("Selected recognition","代表荣誉",l))}<ul class="recognition">{''.join(f'<li><strong>{name}</strong><p>{detail}</p></li>' for name,detail in recognition(l))}</ul></section>'
...
def build():
    ...
    routes=[('',home,('Home','首页')),('recognition',lambda l:official_credits(l,True),('Contributions','开源贡献')),('research',research,('Research','研究')),('publications',publications,('Publications','论文')),('projects',projects,('Projects','项目')),('experience',experience,('Experience','经历')),('contact',contact,('Contact','联系')),('cv',cv,('CV','简历'))]
