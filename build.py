...
def img(name,alt,cls='',eager=False):
    return f'<img src="/assets/{name}" alt="{esc(alt)}" class="{cls}" loading="{"eager" if eager else "lazy"}" decoding="async">'
...
def research(l):
    ...
    for r in RESEARCH:s+=f'<article id="theme-{r[0]}"><span class="research-number">{r[0]}</span><div><h2>{r[2 if l else 1]}</h2><p>{r[6 if l else 5]}</p></div></article>'
    s+='</div>'
...
def publications(l):return heading(tx('Publications','论文',l),tx('Research, shared.','研究与分享。',l),tx('Two publications — including one co-first-author IEEE conference paper.','两篇研究论文，其中一篇为 IEEE 会议共同第一作者论文。',l))+''.join(paper(p,l,True) for p in PAPERS)
