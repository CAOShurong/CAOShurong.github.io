...
PAPERS = [
 ...
 role=('Fourth author','第四作者'),note=('* Tianwu Lei and Bohan Wang contributed equally. Author order shown for the arXiv version.','* 雷天悟与王柏涵同等贡献。此处按 arXiv 版本列出作者。'),image='texture-samples.webp',alt=('Representative cloth, wafer, and metal surface textures from Texture-AD','Texture-AD 中具有代表性的织物、晶圆与金属表面纹理'),
 ...
 abstract=('Texture-AD examines the gap between algorithm development and production-line inspection through representative textures acquired under different optical schemes. The benchmark supports evaluation of how methods generalize across products. The CVM 2026 listing orders the authors as Bohan Wang, Tianwu Lei, Silin Chen, Shurong Cao, and Ningmu Zou; I remain the fourth author.','Texture-AD 通过不同光学方案下采集的代表性纹理，研究算法开发数据与真实产线检测之间的差异，并支持评估方法在不同产品间的泛化能力。CVM 2026 列出的作者顺序为王柏涵、雷天悟、陈思霖、曹书嵘、邹宁睦；我仍为第四作者。'),
...
PROJECTS = [
 ...
 dict(id='vacuum-sensor',title=('Vacuum MOSFET sensing','真空 MOSFET 传感'),tag=('Device physics · Simulation','器件物理 · 仿真'),image=None,
 ...
 dict(id='benchlineage',title=('BenchLineage','BenchLineage'),tag=('Maintainer · Research software','维护者 · 科研软件'),image=None,
